...
{
mulai = 0;
    if((MotionStatus::FALLEN != STANDUP && m_cur_mode == SOCCER && m_is_started == 1)||(MotionStatus::FALLEN != STANDUP && m_cur_mode == MOTION && m_is_started == 1))
    {
        ...
        if(MotionStatus::FALLEN == FORWARD)
            Action::GetInstance()->Start(196);   // FORWARD GETUP
        else if(MotionStatus::FALLEN == BACKWARD)
            Action::GetInstance()->Start(195);   // BACKWARD GETUP
	else if(MotionStatus::FALLEN == RIGHT)
            Action::GetInstance()->Start(61);   // RIGHT GETUP
	else if(MotionStatus::FALLEN == LEFT)
            Action::GetInstance()->Start(60);   // LEFT GETUP
     ...
     if(m_old_btn & BTN_START)
    {
        if(m_is_started == 0)
        {
            fprintf(stderr, "Pemilihan Mode dipilih ");
        ...
        {
            fprintf(stderr, "Sudah bisa dimulai \n");
        }
    }
}
