...
//int h_min = 23, h_max = 104, s_min = 13, s_max = 255, v_min = 41, v_max = 255,conture = 100; //regional
int h_min = /*39*/33, h_max = /*107*/88, s_min = 25/*34*/, s_max = 255, v_min = 41, v_max = 255,conture = 100; //training
//int h_min = 255, h_max = 255, s_min = 255, s_max = 255, v_min = 255, v_max = 255,conture = 100;
 Mat drawing, drawing2, gambar;

//========================= variabel bola ===================================//

int h_min1 = 0, h_max1 = 25, s_min1 =128, s_max1 = 255, v_min1 = 133, v_max1 = 255;
...
//berhenti=1;
	...
	if(  orientasi>=7 && orientasi<=245  ){
			   ...
			   if((orientasi>=0 && orientasi<=6 ) || (orientasi>=482&& orientasi<=600 )) {jadi=2;
							keluar =11;}
							//lurus=11;}
...
//berhenti=1;
			   ...
			   if((orientasi>=158 && orientasi<=160 ) ) {jadi=22;
							keluar =21;}
							//lurus=11;}
			   //=======================motionkeliling tanpa bola======================
					 //muter kiri
...
//berhenti=1;
			   ...
			   if((orientasi>=250 && orientasi<=260 ) ) {jadi=32;
							keluar =31;}
							//lurus=11;}
			   //=======================motionkeliling tanpa bola======================
					 //muter kiri
...
//berhenti=1;
			   ...
			   if((orientasi>=350 && orientasi<=360 ) ) {jadi=42;
							keluar =41;}
							//lurus=11;}
			   //=======================motionkeliling tanpa bola======================
					 //muter kiri
...
//berhenti=1;
			   ...
			   if((orientasi>=250 && orientasi<=260 ) ) {
...
// //cout << "=============================" << endl;
							...
							pos.X=x;//-25;
							pos.Y=y;//-25;
							Point2D center = Point2D(170,110);//(85, 70);
							...
							offset.X *= ((170 / 110)*0.11); // pixel per angle
							offset.Y *= ((170 / 110)*0.11); // pixel per angle
...
ht = 0;
ht2 = 1;
char abj_robot = konek( 'a', tilt); //char konek(argc1, argv1, char robot, int perintah)
		cout << "robot terdekat = " << abj_robot << endl;
	if(abj_robot != 'a'){
	Walking::GetInstance()->Stop();
	
		while(1){
		sleep(2);
		abj_robot = konek( 'a', tilt);
		if(abj_robot != 'b')break;
		}
	}
...
//cout<<"bola dapat gan"<<endl;
					...
					Head::GetInstance()->MoveByAngle(ty,td); //ty yaitu pan, td yaitu tilt
 pan=MotionStatus::m_CurrentJoints.GetAngle(JointData::ID_HEAD_PAN);
 tilt = MotionStatus::m_CurrentJoints.GetAngle(JointData::ID_HEAD_TILT);	
...
if(  orientasi>=7 && orientasi<=245  ){//(orientasi>=0 && orientasi<=75){
				 mode_rotasi=1;  
			   } else if( orientasi>245 && orientasi <470){//< 465){//399){//(orientasi>75 && orientasi <= 150){ 
				 mode_rotasi=0; 
			   } 
			   ...
			   if((tilt<=-3 && orientasi>=0 && orientasi<=6 ) || (tilt<=-3 &&orientasi>=470&& orientasi<=600 ))ey = 1;
			   //=======================motion keliling bola======================
				...
				Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true, true);
						...
						Walking::GetInstance()->X_MOVE_AMPLITUDE =3;//-3;// -3;
						Walking::GetInstance()->Y_MOVE_AMPLITUDE= 46;//54;//41;//52;//54;//70;
						
						Walking::GetInstance()->A_MOVE_AMPLITUDE =8; //16;//-12;//-20;
					...
					Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true, true);
						...
						Walking::GetInstance()->P_OFFSET=-2.0;
						Walking::GetInstance()->PERIOD_TIME=600;//552;//600; 
						Walking::GetInstance()->X_MOVE_AMPLITUDE = 3;//-3;
						Walking::GetInstance()->Y_MOVE_AMPLITUDE= -41;//-45;//-52;//-70;
						Walking::GetInstance()->A_MOVE_AMPLITUDE = -10;//-11;//20;
...
//			  Walking::GetInstance()->Start();
			...
			if(sinyal_masuk == 1){
			 ...
			 if(hitung <= 250){
			  ...
			  Walking::GetInstance()->Start();
			  Walking::GetInstance()->Z_MOVE_AMPLITUDE = 39; 
			  Walking::GetInstance()->PERIOD_TIME=600;//650;
			  hitung++;
			 }
			...
			if(pl == 2)
			{
				...
				{	
					...
					Walking::GetInstance()->PERIOD_TIME=620;//650;
					Walking::GetInstance()->Z_OFFSET = 27;
					//Walking::GetInstance()->A_MOVE_AMPLITUDE = -5;
					Walking::GetInstance()->X_MOVE_AMPLITUDE =15;//13; 
					Walking::GetInstance()->Start();
					//sleep(1);
					//*/
				}
		...
		while(1)
				...
				{	
					...
					Walking::GetInstance()->PERIOD_TIME=600;//650;
					Walking::GetInstance()->Z_OFFSET = 28;
					//Walking::GetInstance()->A_MOVE_AMPLITUDE = -5;
					//Walking::GetInstance()->X_MOVE_AMPLITUDE = 10;//13; 
					Walking::GetInstance()->Start();
			...
			{ 
				...
				if(socer_mode)
				{
					...
					//keadaan muter ketika tidak ada bola //
					//*
...
//=================================================
					/*if(noball && keluar==52 && jadi==53 && berhenti==1)
					{
					Walking::GetInstance()->Stop();
					stop3();
					//bangun();
					}
					if(noball && keluar==51 && jadi==52 && berhenti==1 )
					{
...
Walking::GetInstance()->Start();
						...
						{ 
							...
							if (tilt >=25 && (pan <=-16  && pan >=16 ))
							{
//cout <<"masuk pan tinggi" <<endl;
								//tahan=0;
								tambah=0;
								Walking::GetInstance()->HIP_PITCH_OFFSET=30.0;
								Walking::GetInstance()->X_MOVE_AMPLITUDE=10;//27;
...
Walking::GetInstance()->Start();
							...
							if (tilt >=25 && (pan <=15 && pan >=-15))
							{
								...
								if (kaki_kiri >1420 && kaki_kiri <1950 )
								{
...
//cout<<"masuk coiii 28"<<endl;	
								...
								if((kaki_kiri>=1950 && kaki_kiri<=2600  && kaki_kanan <=2160 ))
								{
...
tambah=0;
								...
								if(kaki_kiri <=1420 && kaki_kanan >=2650 )
								{
//cout <<"mau jatuh kedepan " <<endl;	
							...
							//kondisi pengereman pertama bola dekat 
							if (tilt <=27 && tilt >24 )
							{
								tahan=0;
								tambah=0;
...
sleep(1);
												...
												motion(210);//(200);//(155);kiri=200, kanan=202 //indeks motion tendangan
...
sleep(1);
												...
												motion(211);//(168);kiri=200, kanan=202 //indeks motion tendangan
												//muter=0;
												Walking::GetInstance()->Y_OFFSET=23;
												Walking::GetInstance()->Z_OFFSET=35;
												Walking::GetInstance()->A_OFFSET=0.0;
