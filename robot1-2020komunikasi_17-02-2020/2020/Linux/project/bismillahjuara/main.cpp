 ...
 *  Revision : 2020
 *      Author: robotis, Suhu RSCUAD  muhammad Annas, Danu andrean
 */
...
#define PORT 8088 //robot komunikasi
#define BUFSIZE 2048
...
int muter=0;
int berhenti =0;
int temperatur=0, temperatur1=0;
int sampingaccel=0,sampinggyro=0;
int waktu=0,counter=0;
int adabola=0, tidakada =0;
int tegangan=0;
int puterlagi=0;
...
int data_tilt = 0;
int result_kom = 0;
int stop_robot_from_komunikasi = 0;

int komunikasi(int tilt_kom){
	char *ip_server = "10.0.97.241" ;
...
//========================= variabel lapangan ===================================//
 ...
 //int h_min = 34, h_max = 100, s_min = 36, s_max = 255, v_min = 29, v_max = 255,conture = 100;//ASLINYA
//int h_min = 13, h_max = 134, s_min = 13, s_max = 255, v_min = 29, v_max = 255,conture = 100;//SEMUA KELIHARTAN
int h_min = 23, h_max = 104, s_min = 13, s_max = 255, v_min = 41, v_max = 255,conture = 100;//kelihatandiatas ngambang
//int h_min = 23, h_max = 56, s_min = 13, s_max = 255, v_min = 89, v_max = 255,conture = 100;
//int h_min = 255, h_max = 255, s_min = 255, s_max = 255, v_min = 255, v_max = 255,conture = 100;
 Mat drawing, drawing2, gambar;

//========================= variabel bola ===================================//

int h_min1 = 0, h_max1 = 14, s_min1 =20, s_max1 = 255, v_min1 = 180, v_max1 = 255; //liat tangan
//int h_min1 = 0, h_max1 = 14, s_min1 =33, s_max1 = 255, v_min1 = 226, v_max1 = 255; //kadang panjang 21 133 228
//int h_min1 = 0, h_max1 = 80, s_min1 =102, s_max1 = 255, v_min1 = 190, v_max1 = 255;//kuning jelas
//int h_min1 = 0, h_max1 = 97, s_min1 =140, s_max1 = 255, v_min1 = 197, v_max1 = 255;//bismillah jadi, fluks standar
//int h_min1 = 0, h_max1 = 14, s_min1 =33, s_max1 = 255, v_min1 = 237, v_max1 = 255;//fluks tinggi 
//int h_min1 = 0, h_max1 = 31, s_min1 =115, s_max1 = 255, v_min1 = 174, v_max1 = 255;//fluks rendah
//int h_min1 = 0, h_max1 = 20, s_min1 =86, s_max1 = 255, v_min1 = 185, v_max1 = 255;//fluks rendah merah tua kelihatan
Mat threshold_output;
Mat mask, mask2, bl, mn, gbl;
int x = -5,y = -5;	  
//==================================//
//==========================variabel seragam ==========================//
int h_mins = 163, h_maxs = 255, s_mins = 102, s_maxs = 255, v_mins = 30, v_maxs = 255;
...
//=============================== pemanggilan motion=====================//
 void motion(int indek){
	...
	//Action::GetInstance()->m_Joint.SetEnableHeadOnly(false,false);
...
void tbser()
{
	//============================= trackbar seragam ============================//	   
	namedWindow("Trackbar Lapangan", CV_WINDOW_NORMAL);
	createTrackbar("H_MIN: ","Trackbar seragam",&h_mins,255);
	createTrackbar("H_MAX: ","Trackbar seragam",&h_maxs,255);
	createTrackbar("S_MIN: ","Trackbar seragam",&s_mins,255);
	createTrackbar("S_MAX: ","Trackbar seragam",&s_maxs,255);
	createTrackbar("V_MIN: ","Trackbar seragam",&v_mins,255);
	createTrackbar("V_MAX: ","Trackbar seragam",&v_maxs,255);
	//createTrackbar("Conture: ","Trackbar Lapangan",&conture,255);
	//moveWindow("Trackbar Lapangan", 370, 0);
...
void lap(Mat src_s, int jl)
{
  ...
  // erode( threshold_output, threshold_output, getStructuringElement(MORPH_ELLIPSE, Size(8, 8)) );
   ...
   gambar=Mat::zeros( src.size(), CV_8UC1 );
   ...
   for( int i = 0; i< contours.size(); i++ )
      {
		...
		//dilate( sg, sg, getStructuringElement(MORPH_ELLIPSE, Size(20, 20)) );
		...
		imshow("ssg", ssg);
		imshow("sg", sg);
		circle(src, Point(255,255 ), 3, Scalar(255,255,255), -1);
}
...
//===========================kondisi muter cari arah 1 ======================//
void muter_tanpa_bola()
{
     //berhenti=1;
	orientasi = MotionStatus::kompas;

	if(  orientasi>=7 && orientasi<=245  ){
		mode_rotasi=1;  
	} else if( orientasi>245 && orientasi <482){
		mode_rotasi=0; 
	} 
			   
	if((orientasi>=0 && orientasi<=6 ) || (orientasi>=482&& orientasi<=600 ))
	{
		lurus=0;
		jadi=0;
		berhenti=0;
	...
	//lurus=11;}
    //=======================motionkeliling tanpa bola======================
	if( mode_rotasi==0  ){ //muter kiri
	...
	else if( mode_rotasi==1    ) //muter kanan
	{
		Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true, true);
		Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true, true);
...
void muter_tanpa_bola1()
{

	//berhenti=1;
	orientasi = MotionStatus::kompas;

    if((orientasi>=158 && orientasi<=160 ) ) {
		jadi=22;
        keluar =21;
    }
	//lurus=11;}
...
void muter_tanpa_bola2()
{

	//berhenti=1;
	orientasi = MotionStatus::kompas;
			
	if((orientasi>=255 && orientasi<=260 ) ) {
		jadi=32;
		keluar =31;
	}
	//lurus=11;}
...
void muter_tanpa_bola3()
{
	//berhenti=1;
	orientasi = MotionStatus::kompas;
	
	if((orientasi>=355 && orientasi<=360 ) ) {
		jadi=42;
		keluar =41;
	}
	//lurus=11;}
...
void stop()
{
	...
	if(tilt_scan>0)
	{
		//if((pan_scan>=-95)&&(pan_scan<=75))
		if((pan_scan>=-95)&&(pan_scan<=90)) {pan_scan += 5;}
		//if(pan_scan==75)
		if(pan_scan==90){pan_scan=-95; tilt_scan+=20;}
		if(tilt_scan>60)
		//if(tilt_scan>150)
		{
			tilt_scan=-5;  
	...
	{
		 //if(((pan_scan>=-95))&&(pan_scan<=75))
		if(((pan_scan>=-95))&&(pan_scan<=90)){
			pan_scan += 5;
		}
		if(pan_scan==35){
			pan_scan=-60; tilt_scan += 15;
		}
	}

	Head::GetInstance()->MoveByAngle(pan_scan,tilt_scan);
	//pan=MotionStatus::m_CurrentJoints.GetAngle(JointData::ID_HEAD_PAN);
	//tilt = MotionStatus::m_CurrentJoints.GetAngle(JointData::ID_HEAD_TILT);
	if (tidakada >=100)
	{
...
void bola(Mat vdrawing2, int es)
{
	...
	else
	{
		...
		pos.X=x;//-25;
		pos.Y=y;//-25;
		Point2D center = Point2D(170,120);//(85, 70);
		Point2D offset = pos - center;

		offset *= -1;//-1;//-0.9;//0.5//1 // Inverse X-axis, Y-axis
		//offset.X *= ((VIEW_H_ANGLE /340)); //310)); // pixel per angle
		//offset.Y *= ((VIEW_V_ANGLE /300)); //230)); // pixel per angle

		offset.X *= ((170 / 120)*0.11); // pixel per angle
		offset.Y *= ((170 / 120)*0.11); // pixel per angle
...
void motion_cariball(){ 
	...
	Walking::GetInstance()->P_OFFSET=-4.1; 
	tambah=0; 
	//cout << tilt_scan << " tilt_scan---pan_scan " << pan_scan << endl;
	if(tilt_scan>0)
	{
		//if((pan_scan>=-95)&&(pan_scan<=75))
		if((pan_scan>=-95)&&(pan_scan<=95))
			{pan_scan += 5;}
			//if(pan_scan==75)
		if(pan_scan==95)
			{pan_scan=-95; tilt_scan+=20;}
	...
	Head::GetInstance()->MoveByAngle(pan_scan,tilt_scan); //pan_scan yaitu pan, tilt_scan yaitu tilt
	pan=MotionStatus::m_CurrentJoints.GetAngle(JointData::ID_HEAD_PAN);
	tilt = MotionStatus::m_CurrentJoints.GetAngle(JointData::ID_HEAD_TILT);	
...
// ============================== vision seragam ========================//
void seragam(Mat vdrawing2, int es)
 {
	...
	cv::Mat range = cv::Mat::zeros(ssg.size(), CV_8UC1);
	cv::inRange(frmHsv, cv::Scalar(h_mins, s_mins, v_mins),
		cv::Scalar(h_maxs,s_maxs, v_maxs), range);
	// <<<<< Color Thresholding

	// >>>>> Improving the result
	cv::erode(range, range, cv::Mat(), cv::Point(1,1), 2);
	cv::dilate(range, range, cv::Mat(), cv::Point(1,1), 2);
	cv::dilate(range, range, cv::Mat(), cv::Point(2,2), 3);

	cv::imshow("seragam", range);

	// >>>>> Contours detection
	vector<vector<cv::Point> > contours;
	cv::findContours(range, contours, CV_RETR_EXTERNAL,
			 CV_CHAIN_APPROX_NONE);
	// <<<<< Contours detection
	...
	for (size_t i = 0; i < balls.size(); i++)
	{
		cv::drawContours(src, balls, i, CV_RGB(255,0,0), 5);
		// cv::rectangle(src, ballsBox[i], CV_RGB(0,255,0), 2);
		cv::rectangle(src, ballsBox[i], CV_RGB(0,0,255), 3);
...
//============================== parameter pemanggilan banggun ==================//
void bangun(){
	// cout <<"keluar=" <<keluar<< "--jadi=" <<jadi << "---orientasi ="<<orientasi << endl;
	//cout << "bola=" <<adabola <<endl;
	//waw=0;
	//Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true, true);
	sampingaccel=MotionStatus::RL_ACCEL;
	sampinggyro=MotionStatus::RL_GYRO;
	last_acc=MotionStatus::FB_GYRO;
	cm730.ReadWord(1, MX28::P_PRESENT_POSITION_L, &kaki_kiri, 0); //kaki kiri adalah bahu kiri dan kaki kakan adalah kaki kanan 
	cm730.ReadWord(2, MX28::P_PRESENT_POSITION_L, &kaki_kanan, 0);
	cm730.ReadWord(1, MX28::P_PRESENT_TEMPERATURE, &temperatur, 0); //untuk membaca temperature di setiap servo 
	cm730.ReadWord(2, MX28::P_PRESENT_TEMPERATURE, &temperatur1, 0); 
	cm730.ReadWord(1, MX28::P_PRESENT_VOLTAGE, &tegangan, 0);


	//cout << "tegangan = " << tegangan << endl;
	//cout << "temperaturservo2 = " << temperatur1 << endl;
	...
	if(k == 1){
		...
		if((kaki_kiri < 1100)){ //&& last_acc < 5)){
			...
			//MotionManager::GetInstance()->SetEnable(false);
			//cm730.WriteWord(1, MX28::P_TORQUE_ENABLE, 0, 0);
			//cm730.WriteWord(2, MX28::P_TORQUE_ENABLE, 0, 0);
			cm730.WriteWord(3, MX28::P_TORQUE_ENABLE, 0, 0);
			cm730.WriteWord(4, MX28::P_TORQUE_ENABLE, 0, 0);
			cm730.WriteWord(5, MX28::P_TORQUE_ENABLE, 0, 0);
			cm730.WriteWord(6, MX28::P_TORQUE_ENABLE, 0, 0);
			...
			cm730.WriteWord(20, MX28::P_TORQUE_ENABLE, 0, 0);
		...
		if(kaki_kiri  > 2750 && sampingaccel >460 && sampingaccel < 560){
			tendang=0;
			MotionManager::GetInstance()->SetEnable(false);
			Action::GetInstance()->m_Joint.SetEnableBody(false, false);
		...
		if((sampingaccel > 600)){
			...
			if (waktu>=5)
			{
				...
				//cout <<"waktu=" <<waktu <<endl;        		
				// cout << "kanan " << sampingaccel << endl;MotionManager::GetInstance()->SetEnable(false);
				cout<<"kanan =="<< sampingaccel<<endl;
				sleep(1);
				MotionManager::GetInstance()->SetEnable(true);
				motion(60);
				//motion(195);
...
//============================== penentuan arah gawang lawan =====================//	
 void shoot_to_goals(int &ey, int &yy){
	...
	if(  orientasi>=7 && orientasi<=245  ){//(orientasi>=0 && orientasi<=75){
		mode_rotasi=1;  
	} 
	else if( orientasi>245 && orientasi <465){//< 465){//399){//(orientasi>75 && orientasi <= 150){ 
		mode_rotasi=0; 
	} 
	...
	if((tilt<=-1 && orientasi>=0 && orientasi<=4 ) || (tilt<=-1 &&orientasi>=460&& orientasi<=600 ))ey = 1;
	...
	if( mode_rotasi==0  ){
		...
		Walking::GetInstance()->A_MOVE_AMPLITUDE =12; //16;//-12;//-20;
	...
	else if( mode_rotasi==1    )
	{
		...
		Walking::GetInstance()->A_MOVE_AMPLITUDE = -11;//-11;//20;
...
void wasit()
{
	...
	//bola(drawing, 1);
	//imshow("src", src);
	int sinyal_masuk = referee();
	//printf("sinyal = %d -- hitung = %d \n", sinyal_masuk, hitung);
	/*if(sinyal_masuk == 1){
	// program jalan ready di sini
	if(hitung <= 200){
	bangun();
	Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true,true);
	Walking::GetInstance()->PERIOD_TIME=600;//650;
	...
	Walking::GetInstance()->Stop();
	 ...
	 else	Walking::GetInstance()->Start();
...
int main(int argc, char* argv[])//void)
{
	...
	inisialisasibola();
	//inisialisasiseragam();
    	VideoCapture cap; // pembacaan kamera
	cap.open(0);
	//cap.open("outtttt1.avi");
	//========== pengaturan ukuran frame pixel ============//
	cap.set(CV_CAP_PROP_FRAME_WIDTH,340);//340);//420);
	cap.set(CV_CAP_PROP_FRAME_HEIGHT,300);//300);//340);
    ...
    Action::GetInstance()->Start(9);//Start(9); //pemanggilan motion awal
    while(Action::GetInstance()->IsRunning()) usleep(8*1000);
    Head::GetInstance()->m_Joint.SetEnableHeadOnly(true, true); // set join kepala true
	//Head::GetInstance()->m_Joint.SetEnableHeadOnly(false,false);
   	//Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true, true); //set join semua badan gtrue
	Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true,true);
	...
	while(1)
	{
		...
		while(1)
		{
			...
			if(StatusCheck::mulai == 1){break;StatusCheck::mulai = 0;}
				referee ();
		}
		socer_mode=true;
		if (socer_mode==true)
		{
			...
			hitung++;
						...
						else if (sinyal_masuk== 2)
			{
			hitung =51;
			Walking::GetInstance()->Stop();
			}
						else if (sinyal_masuk == 3)
			{
			//Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true,true);
			...
			if(pl == 3) //tendang=2;sleep(10);
			{
				...
				{	
					...
					Walking::GetInstance()->Z_OFFSET=33;
					Walking::GetInstance()->A_OFFSET=0.0;
					Walking::GetInstance()->HIP_PITCH_OFFSET=22.0;
					Walking::GetInstance()->Z_MOVE_AMPLITUDE= 30;
			...
			if(pl == 2)
			{
				...
				{	
					...
					Walking::GetInstance()->PERIOD_TIME=600;//650;
					//Walking::GetInstance()->A_MOVE_AMPLITUDE = -5;
					Walking::GetInstance()->X_MOVE_AMPLITUDE =10;//13; 
					Walking::GetInstance()->Start();
					//sleep(1);
					//*/
				}
			...
			{ 
				...
				if(socer_mode)
				{
					...
					//result_kom = komunikasi(data_tilt);					
					//wasit();
					//tendang=1;
					lap(src, 1);
					bola(drawing, 1);
					...
					//=======================================================stop arah gawang lawan=====================
					//keaadaan ketika tidak melihat bola//
					if(noball){
						if(noball && keluar==10 && jadi==1 )
						{
						...
						if(noball && keluar==11 && jadi==2 && berhenti==1 )
						{
						...
						if(noball && keluar==12 && jadi==3 && berhenti==1)
						{
							Walking::GetInstance()->Stop();
							stop();
							//bangun();
						}
						//============================================================stop arah kanan gawang lawan==========
						
						if(noball && keluar==20 && jadi==1 )
						{
						...
						if(noball && keluar==21 && jadi==22 && berhenti==1 )
						{
						...
						if(noball && keluar==22 && jadi==23 && berhenti==1)
						{
							Walking::GetInstance()->Stop();
							stop1();
							//bangun();
						}
						//===========================================stop arah gawang sendiri======================
						
						if(noball && keluar==30 && jadi==1 )
						{
						...
						if(noball && keluar==31 && jadi==32 && berhenti==1 )
						{
						...
						if(noball && keluar==32 && jadi==33 && berhenti==1)
						{
							Walking::GetInstance()->Stop();
							stop2();
							//bangun();
						}
						//=========================================stop arah kiri gawang lawan================================

						if(noball && keluar==40 && jadi==1 )
						{
						...
						if(noball && keluar==41 && jadi==42 && berhenti==1 )
						{
						...
						if(noball && keluar==42 && jadi==43 && berhenti==1)
						{
							Walking::GetInstance()->Stop();
							stop3();
							//bangun();
						}
						//===========================================muter arah gawang sendiri dan normal======================

						if(noball && keluar==50 && jadi==1 )
						{
							tambah=0;
							jalan = 10;
							berhenti=1;
						 ...
						 //*/
						// keaadan ketika tidak ada bola normal//
						
						if(noball && tendang==1 && jadi==0  && berhenti==0 )
						{
							...
							// Walking::GetInstance()->A_OFFSET=0.0;
							// Walking::GetInstance()->HIP_PITCH_OFFSET=25;
							// Walking::GetInstance()->Z_MOVE_AMPLITUDE= 40;
							// Walking::GetInstance()->BALANCE_KNEE_GAIN=0.02;
							// Walking::GetInstance()->BALANCE_ANKLE_PITCH_GAIN=0.02;
							// Walking::GetInstance()->BALANCE_HIP_ROLL_GAIN=0.02;
						...
						if(noball && tendang==2 && berhenti==0 && jadi ==0)
						{
							//=======================speed movemnt ball scan=================
							//Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true, true);
							tambah=0;
							...
							// Walking::GetInstance()->Z_OFFSET=35;
							// Walking::GetInstance()->A_OFFSET=0.0;
							// Walking::GetInstance()->HIP_PITCH_OFFSET=25.0;
							// Walking::GetInstance()->Z_MOVE_AMPLITUDE= 40;
							// Walking::GetInstance()->BALANCE_KNEE_GAIN=0.02;
							// Walking::GetInstance()->BALANCE_ANKLE_PITCH_GAIN=0.02;
							// Walking::GetInstance()->BALANCE_HIP_ROLL_GAIN=0.02;
						...
						if(noball && tendang==0 && jadi==0 && berhenti==0 )
						{
							//=======================speed movemnt ball scan=================
							Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true, true);
							cout <<tendang <<endl;
							tambah=0;
							jalan = 10;
							//sudah jadi aktifin lurus++ klau mau muter
							lurus++;
							...
							motion_cariball();
							cout << "lurus="<<lurus <<endl;
							if (lurus >=200 && puterlagi==0)
							{
								jadi=1;
								keluar =30;
							}
							else if (lurus>=200 && puterlagi==1)
							{
								jadi=1;
								keluar =10;
						...
						//reset data komunikasi
						data_tilt=0;
						stop_robot_from_komunikasi = 0;
					} //akhir noball
					//keaadaan ketika melihat bola//
					else if(!noball)
					{
						//=================komunikasi=============== //

						if (tilt<15) data_tilt=1;
						else data_tilt=0;
								

						if(result_kom ==1 && tilt <30 ){
							Walking::GetInstance()->Stop();
							Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(false,false);
					
							cout<<"harusnya berhenti0"<<endl<<endl<<endl<<endl;
							stop_robot_from_komunikasi = 1;
						//sleep(10);						
						}
						else{
							//reset stop
							...
							Walking::GetInstance()->m_Joint.SetEnableBodyWithoutHead(true, true);
							Walking::GetInstance()->Z_MOVE_AMPLITUDE= 47;
							//cout <<tendang <<endl;
							...
							cout<< "nilai keseimbangan kaki = "<<kaki_kiri <<"// --tilt ="<<tilt_percent<<endl<<endl;
							...
							// 	//tahan=0;
							// 	tambah=0;
							// 	Walking::GetInstance()->HIP_PITCH_OFFSET=24.0;
							// 	Walking::GetInstance()->X_MOVE_AMPLITUDE=10;//27;
							// 	//m_FBStep = 10;
							// 	Walking::GetInstance()->Start();
							...
							{ 
								...
								if  (pan <=15 && pan >=-15)
								{
									tahan=0;
									Walking::GetInstance()->P_OFFSET=-4.1;
									Walking::GetInstance()->HIP_PITCH_OFFSET=25;//30.4;
									// kondisi keaadan jalan stabil
									if ( tilt >= 25 && (kaki_kiri >1420 && kaki_kiri <1950) )
									{
										...
										if (tambah >=8)
										{
											...
											if (jalan >=30)
											{
												...
												Walking::GetInstance()->P_OFFSET=-4.1;
												Walking::GetInstance()->HIP_PITCH_OFFSET=26.0;//30.4;

												jalan=30; //jalan maksimal 

											} 
											Walking::GetInstance()->X_MOVE_AMPLITUDE =jalan;//27;
                                 // Walking::GetInstance()->HIP_PITCH_OFFSET=24;//30.4;
											cout<<"kondisi sudah stabil " << jalan <<endl;	
									...
									if (tilt <=24 && tilt >15 )
									{
										tahan=0;
										tambah=0;
										cout << "rem pertama" <<endl<<endl;
										//Walking::GetInstance()->Stop();
										//usleep(1000);
										Walking::GetInstance()->P_OFFSET=-4.1;
										Walking::GetInstance()->HIP_PITCH_OFFSET=22.0;

										Walking::GetInstance()->X_MOVE_AMPLITUDE =18;
									
										//muter=1;
									
									}
									//kondisi pengereman kedua bola dekat 
									if (tilt <=15 )
									{
										tahan=0;
										tambah=0;
										cout << "rem kedua" <<endl<<endl;
										//Walking::GetInstance()->Stop();
										//usleep(1000);
										Walking::GetInstance()->P_OFFSET=-4.1;
										Walking::GetInstance()->HIP_PITCH_OFFSET=24;
										Walking::GetInstance()->A_MOVE_AMPLITUDE =0;
										Walking::GetInstance()->X_MOVE_AMPLITUDE =15;
										
									}
								
								} // akhir di dalam jangkauan

								else if (pan <=-16  || pan >=16 )
								{
									cout <<"masuk pan tinggi kurang dari 50" <<endl;
									//tahan=0;
									tambah=0;
									Walking::GetInstance()->P_OFFSET=-4.1;
									Walking::GetInstance()->HIP_PITCH_OFFSET=25.0;
                        ...
                        // ===================keseimbangan=====================
								//kondisi ketika mau jatuh kebelakang00
								if((kaki_kiri>=1980 && kaki_kiri<=2600   ))
								{

									cout <<"mau jatuh kebelakang " <<endl;	
									Walking::GetInstance()->X_MOVE_AMPLITUDE = 0;
									Walking::GetInstance()->P_OFFSET=-5.0;
									Walking::GetInstance()->HIP_PITCH_OFFSET=30.0;//30.4;
									jalan=10;
									tambah=0;

								}
								//kondisi ketika mau jatuh kedepan
								if(kaki_kiri <=1480 && kaki_kiri >=1380  )
								{
									cout <<"mau jatuh kedepan " <<endl;	

									// Walking::GetInstance()->X_MOVE_AMPLITUDE = 28 ;
									Walking::GetInstance()->P_OFFSET=-3.2;
									Walking::GetInstance()->HIP_PITCH_OFFSET=22.0;//30.4;
									tambah=0;
									jalan=10;

								}
								else if(kaki_kiri <=1380)
								{
									tambah=0;	
									cout << " mau jatuh depan penuh" <<endl<<endl;
									Walking::GetInstance()->P_OFFSET=-2;
									Walking::GetInstance()->HIP_PITCH_OFFSET=20.0;
                        ...
                        // -------------------------------------------------------------------------------------------------------------
								...
								//if (tilt <=20 )//&& (pan <=10 && pan >=-10))
								{
									...
									// kondisi ketika bola dekat tetapi tidak terjangkau kaki
									if (tilt<=-5 && pan > 15)
									{
										Walking::GetInstance()->HIP_PITCH_OFFSET=21.0;
										Walking::GetInstance()->P_OFFSET=-3.7;
									...
									else if (tilt<=-5 && pan < -15)
									{
										Walking::GetInstance()->HIP_PITCH_OFFSET=21.0;
										Walking::GetInstance()->P_OFFSET=-3.7;

										Walking::GetInstance()->A_MOVE_AMPLITUDE = 16;
										Walking::GetInstance()->X_MOVE_AMPLITUDE = -2;
									...
									if (tilt<=-4 && (pan>=-15 && pan<=15) && y >= 90)
									{
										...
										shoot_to_goals(kh, x); //penentuaan arah gawang lawan
										if( kh == 1 ){ww = 1;kh = 0;}
										//cout<< "tendang >> y=" << y <<endl;	
										if(ww == 1)
										{
											//if( x >= 170)//85) //kanan
											//penetuan tendangan kanan atau kiri
											if( pan <= 0)//85) //tendangan kanan
											{
												...
												motion(202);//(200);//(155);kiri=200, kanan=202 //indeks motion tendangan
												...
												Walking::GetInstance()->A_OFFSET=0.0;
												Walking::GetInstance()->P_OFFSET=-4.1;
												Walking::GetInstance()->HIP_PITCH_OFFSET=24.0;//30.4;
												Walking::GetInstance()->Z_MOVE_AMPLITUDE= 40;
												Walking::GetInstance()->BALANCE_KNEE_GAIN=0.02;
												Walking::GetInstance()->BALANCE_ANKLE_PITCH_GAIN=0.02;
												Walking::GetInstance()->BALANCE_HIP_ROLL_GAIN=0.02;
											...
											else if(pan >0) // tendangan kiri 
											{
												...
												motion(200);//(168);kiri=200, kanan=202 //indeks motion tendangan
												//muter=0;
												Walking::GetInstance()->X_OFFSET=5;	
												Walking::GetInstance()->Z_OFFSET=35;
												Walking::GetInstance()->A_OFFSET=0.0;
							...
							else
							{
								...
								if  (pan >= 50 || pan <=-50)
								{
									cout <<"pan sangat tinggi" <<endl;
									//tahan=0;
									tambah=0;
                           Walking::GetInstance()->P_OFFSET=-4.0;
									Walking::GetInstance()->HIP_PITCH_OFFSET=24.0;
									Walking::GetInstance()->X_MOVE_AMPLITUDE=10;//27;
									Walking::GetInstance()->Start();
						...
						}//akhir else komunikasi		
					} //akhir !noball
					if (stop_robot_from_komunikasi == 0 )
					{
					...
					} //akhir stop_robot_from_komunikasi
				}
			x = -5;
			y = -5;
			//	*/
