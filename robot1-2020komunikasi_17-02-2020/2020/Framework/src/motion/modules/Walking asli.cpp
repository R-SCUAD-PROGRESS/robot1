...
Walking::Walking()
{
	...
	BALANCE_ANKLE_PITCH_GAIN = 0.5;
	BALANCE_HIP_ROLL_GAIN = 0.4;
	BALANCE_ANKLE_ROLL_GAIN = 0.5;
	...
	m_Joint.SetAngle(JointData::ID_R_SHOULDER_PITCH, -0);
	m_Joint.SetAngle(JointData::ID_L_SHOULDER_PITCH, 0);
...
{
	return mag * sin(2 * 3.141592 / period * time - period_shift) + (mag_shift/2);
}
...
void Walking::update_param_time()
{
    ...
    m_Pelvis_Swing = m_Pelvis_Offset ;
...
void Walking::Process()
{
    ...
    double initAngle[14] = {   0.0,       3.0,        0.0,       0.0,        0.0,          3.0,         0.0,       -3.0,        2.0,        -2.0,       2.0,          -3.0,       -15,       15    };
	int outValue[14];
   ...
   if(BALANCE_ENABLE == true)
    ...
    { 
		...
		double rlAccoErr = MotionStatus::RL_ACCEL;
		double fbAccoErr = MotionStatus::FB_ACCEL;
		double rlGyroErr1 = int (rlGyroErr - 512);
		double fbGyroErr1 = int (fbGyroErr - 512);
       
	       float fbAccoErrz = int (fbAccoErr- 470);
               float fbAccoErrs = int (fbAccoErr- 470);
               float fbAccoErr1 = (0.02*(fbAccoErrz-fbAccoErr1))*75;
               float rlAccoErr2 = (0.02*(rlAccoErr-rlAccoErr2));
               float max_condongbel_acc=490;
                float max_condongdep_acc=450;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
	outValue[3] -= (int)(dir[3] * (fbGyroErr * BALANCE_KNEE_GAIN*2)*2); // R_KNEE//-
        outValue[9] -= (int)(dir[9] * (fbGyroErr * BALANCE_KNEE_GAIN*2)*2); // L_KNEE//-

	outValue[4] -= (int)((dir[4] * fbGyroErr * BALANCE_ANKLE_PITCH_GAIN*2)*2); // R_ANKLE_PITCH//-
        outValue[10] -= (int)((dir[10] * fbGyroErr * BALANCE_ANKLE_PITCH_GAIN*2)*2); // L_ANKLE_PITCH//-
	...
	outValue[5] -= (int)(dir[5] * rlGyroErr * BALANCE_ANKLE_ROLL_GAIN*4); // R_ANKLE_ROLL//-
        ...
        outValue[12] -= (int)((dir[12] * fbAccoErr1/0.8)*8); // R_ANKLE_ROLL
        outValue[13] -= (int)((dir[13] * fbAccoErr1/0.8)*8);        
	///////////////////////////////////////////////////////////////////////////////////////////////////
	//outValue[14] += (int)((dir[14] * fbAccoErr1/8)*4); // R_ANKLE_ROLL
        //outValue[15] -= (int)((dir[15] * fbAccoErr1/8)*4);
	/*outValue[3] -= (int)(dir[3] * (fbGyroErr * BALANCE_KNEE_GAIN)*2); // R_KNEE//-
        outValue[9] -= (int)(dir[9] * (fbGyroErr * BALANCE_KNEE_GAIN)*2);
       ...
       outValue[12] -= (int)((dir[12] * fbAccoErr1/0.8)*4); // R_ANKLE_ROLL
       outValue[13] -= (int)((dir[13] * fbAccoErr1/0.8)*4);
  */

    /* 
   	
if(fbAccoErr<=max_condongdep_acc){
      		outValue[2] -= (int)(dir[2]*40); 
        	outValue[8] -= (int)(dir[8]*40);
           	m_Joint.SetPGain(JointData::ID_R_HIP_PITCH, 2);
        	m_Joint.SetIGain(JointData::ID_R_HIP_PITCH, 10);
        	m_Joint.SetDGain(JointData::ID_R_HIP_PITCH, 0);

        	m_Joint.SetPGain(JointData::ID_L_HIP_PITCH, 2);
        	m_Joint.SetIGain(JointData::ID_L_HIP_PITCH, 10);
        	m_Joint.SetDGain(JointData::ID_L_HIP_PITCH, 0);
       }
  	else  if((fbAccoErr<=(max_condongdep_acc+10)) && (fbAccoErr>=max_condongdep_acc) ){
     		 outValue[2] -= (int)(dir[2]*20); 
       		 outValue[8] -= (int)(dir[8]*20);
           	m_Joint.SetPGain(JointData::ID_R_HIP_PITCH, 10);
        	m_Joint.SetIGain(JointData::ID_R_HIP_PITCH, 10);
        	m_Joint.SetDGain(JointData::ID_R_HIP_PITCH, 0);


       		m_Joint.SetPGain(JointData::ID_L_HIP_PITCH, 10);
        	m_Joint.SetIGain(JointData::ID_L_HIP_PITCH, 10);
        	m_Joint.SetDGain(JointData::ID_L_HIP_PITCH, 0);
       	} 
     	
//====================================feedback condong belakang===========================

        if(fbAccoErr>=max_condongbel_acc){
      		outValue[2] -= (int)(dir[2]*-40); 
        	outValue[8] -= (int)(dir[8]*-40);
           	m_Joint.SetPGain(JointData::ID_R_HIP_PITCH, 2);
        	m_Joint.SetIGain(JointData::ID_R_HIP_PITCH, 10);
  	...
  	else  if((fbAccoErr>=(max_condongbel_acc-10)) && (fbAccoErr<=max_condongbel_acc) ){
     		 outValue[2] -= (int)(dir[2]*-20); 
       		 outValue[8] -= (int)(dir[8]*-20);
           	m_Joint.SetPGain(JointData::ID_R_HIP_PITCH, 10);
        	m_Joint.SetIGain(JointData::ID_R_HIP_PITCH, 10);
        	...
        	m_Joint.SetDGain(JointData::ID_L_HIP_PITCH, 0);
       		 ...
       		 outValue[2] -= (int)(dir[2]); 
        	outValue[8] -= (int)(dir[8]);
        	m_Joint.SetPGain(JointData::ID_R_HIP_PITCH, 20);
        	m_Joint.SetIGain(JointData::ID_R_HIP_PITCH, 10);
        	...
        	m_Joint.SetPGain(JointData::ID_L_HIP_PITCH, 20);
        	m_Joint.SetIGain(JointData::ID_L_HIP_PITCH, 10);
       		m_Joint.SetDGain(JointData::ID_L_HIP_PITCH, 0);}
...
#else
	//outValue[1] -= (int)(dir[1] * rlGyroErr * BALANCE_HIP_ROLL_GAIN*4); // R_HIP_ROLL//td +
       // outValue[7] -= (int)(dir[7] * rlGyroErr * BALANCE_HIP_ROLL_GAIN*4); // L_HIP_ROLL//+
	////////////////////////////////////////////////////////////////////////////////////////////////////
        outValue[3] += (int)(dir[3] * (fbGyroErr * BALANCE_KNEE_GAIN*2)*2); // R_KNEE//-
        outValue[9] += (int)(dir[9] * (fbGyroErr * BALANCE_KNEE_GAIN*2)*2); // L_KNEE//-

	outValue[4] += (int)((dir[4] * fbGyroErr * BALANCE_ANKLE_PITCH_GAIN*2)*4); // R_ANKLE_PITCH//-
        outValue[10] += (int)((dir[10] * fbGyroErr * BALANCE_ANKLE_PITCH_GAIN*2)*4); // L_ANKLE_PITCH//-

	outValue[5] += (int)(dir[5] * rlGyroErr * BALANCE_ANKLE_ROLL_GAIN*4); // R_ANKLE_ROLL//-
        outValue[11] += (int)(dir[11] * rlGyroErr * BALANCE_ANKLE_ROLL_GAIN*4); // L_ANKLE_ROLL//-
        
        outValue[12] += (int)((dir[12] * fbAccoErr1/1.8)*12); // R_ANKLE_ROLL
        outValue[13] += (int)((dir[13] * fbAccoErr1/1.8)*12);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	m_Joint.SetAngle(JointData::ID_HEAD_PAN, A_MOVE_AMPLITUDE);
  		...
  		//m_Joint.SetPGain(JointData::ID_R_HIP_PITCH, 15);//20);
        	//m_Joint.SetIGain(JointData::ID_R_HIP_PITCH, 10);
        	//m_Joint.SetDGain(JointData::ID_R_HIP_PITCH, 0);


        	//m_Joint.SetPGain(JointData::ID_L_HIP_PITCH, 15);//20);
        	//m_Joint.SetIGain(JointData::ID_L_HIP_PITCH, 10);
        	//m_Joint.SetDGain(JointData::ID_L_HIP_PITCH, 0);

//==============================================angkle roll==========================
        m_Joint.SetPGain(JointData::ID_L_ANKLE_ROLL, 20);//30);
        m_Joint.SetIGain(JointData::ID_L_ANKLE_ROLL, 10);
        m_Joint.SetDGain(JointData::ID_L_ANKLE_ROLL, 0);
        m_Joint.SetPGain(JointData::ID_R_ANKLE_ROLL, 20);//30);
        m_Joint.SetIGain(JointData::ID_R_ANKLE_ROLL, 10);
        m_Joint.SetDGain(JointData::ID_R_ANKLE_ROLL, 0);
//==================================angkle pitch========================== (mata kaki)
        m_Joint.SetPGain(JointData::ID_L_ANKLE_PITCH, 25);
        m_Joint.SetIGain(JointData::ID_L_ANKLE_PITCH, 10);
        m_Joint.SetDGain(JointData::ID_L_ANKLE_PITCH, 0);
        m_Joint.SetPGain(JointData::ID_R_ANKLE_PITCH, 25);
        m_Joint.SetIGain(JointData::ID_R_ANKLE_PITCH, 10);
        m_Joint.SetDGain(JointData::ID_R_ANKLE_PITCH, 0);
//===================================knee================================= (dengkul)
        m_Joint.SetPGain(JointData::ID_L_KNEE, 20);
        m_Joint.SetIGain(JointData::ID_L_KNEE, 10);
        m_Joint.SetDGain(JointData::ID_L_KNEE, 0);
        m_Joint.SetPGain(JointData::ID_R_KNEE, 20);
        m_Joint.SetIGain(JointData::ID_R_KNEE, 10);
        m_Joint.SetDGain(JointData::ID_R_KNEE, 0);
//======================================hip roll==========================
        m_Joint.SetPGain(JointData::ID_L_HIP_ROLL, 20);//30);
        m_Joint.SetIGain(JointData::ID_L_HIP_ROLL, 10);
        m_Joint.SetDGain(JointData::ID_L_HIP_ROLL, 0);
        m_Joint.SetPGain(JointData::ID_R_HIP_ROLL, 20);//30);
        m_Joint.SetIGain(JointData::ID_R_HIP_ROLL, 10);
        m_Joint.SetDGain(JointData::ID_R_HIP_ROLL, 0);
//=======================================hip yaw==========================
        m_Joint.SetPGain(JointData::ID_L_HIP_YAW, 20);
        m_Joint.SetIGain(JointData::ID_L_HIP_YAW, 10);
        m_Joint.SetDGain(JointData::ID_L_HIP_YAW, 0);
        m_Joint.SetPGain(JointData::ID_L_HIP_YAW, 20);
        m_Joint.SetIGain(JointData::ID_R_HIP_ROLL, 10);
        m_Joint.SetDGain(JointData::ID_L_HIP_YAW, 0);
...
//================================tangan FB=================================
        ...
        m_Joint.SetIGain(JointData::ID_R_SHOULDER_PITCH, 10);
        m_Joint.SetDGain(JointData::ID_R_SHOULDER_PITCH, 0);

        m_Joint.SetPGain(JointData::ID_L_SHOULDER_PITCH, 2);
        m_Joint.SetIGain(JointData::ID_L_SHOULDER_PITCH, 10);
        m_Joint.SetDGain(JointData::ID_L_SHOULDER_PITCH, 0);

        
       
//================================tangan RL=================================
  /*      m_Joint.SetPGain(JointData::ID_R_SHOULDER_ROLL, 20);
        m_Joint.SetIGain(JointData::ID_R_SHOULDER_ROLL, 10);
        m_Joint.SetDGain(JointData::ID_R_SHOULDER_ROLL, 0);

        m_Joint.SetPGain(JointData::ID_L_SHOULDER_ROLL, 20);
        m_Joint.SetIGain(JointData::ID_L_SHOULDER_ROLL, 10);
        m_Joint.SetDGain(JointData::ID_L_SHOULDER_ROLL, 0);
