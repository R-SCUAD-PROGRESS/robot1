...
Walking::Walking()
{
	...
	m_Joint.SetAngle(JointData::ID_R_SHOULDER_PITCH, -3);
	m_Joint.SetAngle(JointData::ID_L_SHOULDER_PITCH, 3);
	m_Joint.SetAngle(JointData::ID_R_SHOULDER_ROLL, -13.873);
    m_Joint.SetAngle(JointData::ID_L_SHOULDER_ROLL, 17.580);
	m_Joint.SetAngle(JointData::ID_R_ELBOW, 19.300);
	m_Joint.SetAngle(JointData::ID_L_ELBOW, -19.593);
...
void Walking::Process()
{
    ...
    double initAngle[14] = {   3.0,       4.0,        -8.0,       12.0,        2.0,          3.0,         -3.0,       -3.0,        8.0,        -12.0,       -2.0,          -3.0,       -3,	0 };
	int outValue[14];
   ...
   if(BALANCE_ENABLE == true)
    ...
    { 
		...
		//double rlGyroErr1 = int (rlGyroErr - 100);
		//double fbGyroErr1 = int (fbGyroErr - 100);
       
	       float fbAccoErrz = int (fbAccoErr- 512);
               float fbAccoErrs = int (fbAccoErr- 512);
	...
	outValue[3] -= (int)(dir[3] * (fbGyroErr * BALANCE_KNEE_GAIN*4)/4); // R_KNEE//-
        outValue[9] -= (int)(dir[9] * (fbGyroErr * BALANCE_KNEE_GAIN*4)/4); // L_KNEE//-

	outValue[4] -= (int)((dir[4] * fbGyroErr * BALANCE_ANKLE_PITCH_GAIN*2)/9); // R_ANKLE_PITCH//-
        outValue[10] -= (int)((dir[10] * fbGyroErr * BALANCE_ANKLE_PITCH_GAIN*2)/9); // L_ANKLE_PITCH//-*/

	outValue[5] -= (int)(dir[5] * (rlGyroErr * BALANCE_ANKLE_ROLL_GAIN*2)/9); // R_ANKLE_ROLL//-
        outValue[11] -= (int)(dir[11] * (rlGyroErr * BALANCE_ANKLE_ROLL_GAIN*2)/9); // L_ANKLE_ROLL//- */
        
        outValue[12] -= (int)((dir[12] * fbAccoErr1*6)/2.5); // R_ANKLE_ROLL
        outValue[13] -= (int)((dir[13] * fbAccoErr1*6)/2.5);        
