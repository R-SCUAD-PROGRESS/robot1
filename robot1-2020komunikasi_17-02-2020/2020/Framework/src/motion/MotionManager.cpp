...
void MotionManager::Process()
{
    ...
    if(m_CalibrationStatus == 1 && m_Enabled == true)
    {
	 ...
	 int m_kompas = 0;
		...
		if(m_CM730->ReadWord(CM730::ID_CM, CM730::P_ADC15_L, &value, 0) == CM730::SUCCESS){
	    MotionStatus::kompas = value - m_kompas;}
	    ...
	    MotionStatus::RL_ACCEL = m_CM730->m_BulkReadData[CM730::ID_CM].ReadWord(CM730::P_ACCEL_Y_L);
            rl_array[buf_idxrl] = MotionStatus::RL_ACCEL;
            if(++buf_idxrl >= ACCEL_WINDOW_SIZE) buf_idxrl = 0;

            MotionStatus::FB_ACCEL = m_CM730->m_BulkReadData[CM730::ID_CM].ReadWord(CM730::P_ACCEL_X_L);
            fb_array[buf_idx] = MotionStatus::FB_ACCEL;
            if(++buf_idx >= ACCEL_WINDOW_SIZE) buf_idx = 0;
        }

        int sum = 0, avr = 512,sim=0,rl=512;
        for(int idx = 0; idx < ACCEL_WINDOW_SIZE; idx++){
            sum += fb_array[idx];
            sim += rl_array[idx];}
        avr = sum / ACCEL_WINDOW_SIZE;
        rl = sim / ACCEL_WINDOW_SIZE;
	...
	else if(rl > MotionStatus::FALLEN_L_LIMIT)
            MotionStatus::FALLEN = LEFT;
	else if(rl < MotionStatus::FALLEN_R_LIMIT)
